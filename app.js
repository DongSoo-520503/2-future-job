...
// 루트 접속 시 index.html 반환 (정적만으로도 되지만 확실히 고정)
app.get('/', (req, res) => {
  res.sendFile(path.join(__dirname, 'index.html'));
});

// ----- 데이터 로딩 (Render에서도 동일 경로) -----
const dataPath = path.join(__dirname, 'data', 'future_job_3960.json');
let jobData = [];

function normalizeRowKeys(row) {
  // 키에 붙은 공백/탭 등을 제거해 "직업해설 " 같은 문제를 방지
  const out = {};
...
try {
  ...
  jobData = Array.isArray(raw) ? raw.map(normalizeRowKeys) : [];
  console.log(`[DATA] Loaded rows: ${jobData.length}`);
} catch (err) {
  console.error('[DATA] Failed to load JSON:', err.message);
  // Render에서 데이터 파일 누락/경로 오류를 바로 알 수 있게 서버는 뜨되 에러를 남김
  jobData = [];
}
...
function calcScore(desc, keywords) {
  ...
  const rawScore = keywords.reduce((sum, kw) => {
    const count = String(desc).split(kw).length - 1;
...
app.post('/recommend', (req, res) => {
  ...
  if (!jobData.length) {
    return res.status(500).json({
      error:
        '서버 데이터(future_job_3960.json)를 불러오지 못했습니다. data 폴더/파일 경로 및 배포 포함 여부를 확인하세요.',
    });
  }
  ...
  candidates.forEach((row) => {
    // ✅ "직업해설" 키가 조금 달라도 찾도록 방어
    const desc = pick(row, ['직업해설', '직업 해설'], '');
  ...
  // ✅ 여기서 undefined 나던 필드들 방어적으로 pick
  const bestJob = pick(best, ['추천직업'], '(데이터 없음)');
  const bestRank = pick(best, ['연봉순위'], '(데이터 없음)');

  const jobDesc = pick(best, ['직업해설', '직업 해설'], '(데이터 없음)');

  // 핵심 전문지식: JSON에서 띄어쓰기 버전이 다를 수 있으니 모두 대응
  const coreKnow = pick(best, ['핵심 전문지식', '핵심 전문 지식', '핵심전문지식'], '(데이터 없음)');

  const major = pick(best, ['추천 학과/전공', '추천학과/전공'], '(데이터 없음)');
  const prep = pick(best, ['준비 기간', '준비기간'], '(데이터 없음)');
...
추천직업: ${bestJob}
연봉순위: ${period} ${country},"지능/성적/노력"을 고려한 20개 해당 직종 중 ${bestRank}위에 해당됩니다.
직업해설: ${jobDesc}
핵심 전문지식: ${coreKnow}
추천 학과/전공: ${major}
준비 기간: ${prep}
...
성향 점수화: 직업흥미유형(RIASEC) 적합도 ${best.riasec_score.toFixed(
    1
  )}점, 개인성향(Big5) 적합도 ${best.big5_score.toFixed(
    1
  )}점을 60:40 가중평균하여 최종 적합도 ${best.final_score}점을 산출하였습니다.
